...
public class LoginAttemptService {
    ...
    public LoginAttemptService(){
        super();
        loginAttemptCache = CacheBuilder.newBuilder().expireAfterAccess(15,MINUTES)
...
public boolean hasExceededMaxAttempts(String usenaime) {
    try {
        return  loginAttemptCache.get(usenaime)>= MAXIMUM_NUMBER_OF_ATTEMPTS;
    } catch (ExecutionException e) {
        e.printStackTrace();
    }
    return false;
